"""Shared CSV-upload processing (parse -> validate -> optional zip->DMA ->
densify -> GeoDataRead validation) used both for a standalone dataset
upload and for attaching a refreshed dataset to an existing experiment.
"""
...
def process_dataset_upload(
    *,
    ...
    storage_path = storage.save_bytes(subdir=f"datasets/{org_id}", filename=file.filename, content=content)
