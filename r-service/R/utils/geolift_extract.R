# Builds the JSON-ready result (summary, att_series, lift_series,
# cumulative_effect_series, weights) from a fitted GeoLift::GeoLift() object.
# Shared by analyze.R (a real post-test run) and simulate.R (a hypothetical
# pre-test run against a modified copy of the data) so both endpoints stay
# in sync on exactly what gets extracted and how.
extract_geolift_result <- function(gl, body) {
